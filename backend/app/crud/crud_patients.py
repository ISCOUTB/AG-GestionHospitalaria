...
class CRUDPatients(CRUDBase):
    def get_all_patients(self, db: Session, active: bool = True) -> list[schemas.PatientAll]:
        """
        ...
        """
        stmt = self.__join_patients(active)
        query = db.execute(stmt).all()
        results: list[schemas.PatientAll] = []
        for row in query:
            userbase = self.__create_user_base(row[:9])
            responsable_info = self.__create_patient_info(row[9:])
    ...
    def get_patient(self, num_document: str, db: Session, active: bool = True) -> schemas.PatientAll | None:
        ...
        """
        stmt = self.__join_patients(active).where(models.UsersInfo.num_document == num_document)
        query = db.execute(stmt).first()
        if not query:
            return None

        userbase = self.__create_user_base(query[:9])
        responsable_info = self.__create_patient_info(query[9:])
        return schemas.PatientAll(
            **userbase.model_dump(),
            **responsable_info.model_dump()
        )
    
    def add_responsable(self, num_document: str, responsable_info: schemas.ResponsablesInfo, db: Session) -> int:
        """
        ...
        """
        valid_responsable: int = self.__valid_responsable_doc(num_document, responsable_info.num_doc_responsable, db)
    ...
    def update_patient(self, num_document: str, updated_info: schemas.ResponsablesInfo, db: Session) -> int:
        """
        ...
        Returns:
            int: Retorna un entero simbolizando el estado de la respuesta. Estos son los posibles estados de la respuesta:
                ...
                - 4: Información previa del paciente no existente.
        """
        if updated_info.num_doc_responsable is not None and \
            (out := self.__valid_responsable_doc(num_document, updated_info.num_doc_responsable, db)) != 0:
            return out
