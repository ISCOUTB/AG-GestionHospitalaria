...
def test_create_user(client: TestClient, superuser_token: dict[str, str], db: Session) -> None:
    ...
    user_search = schemas.UserSearch(
        num_document=admin.num_document,
        rol='admin'
    )
...
def test_create_user_non_superuser(client: TestClient, non_superuser_token: dict[str, str]) -> None:
    admin = schemas.UserCreate(
        num_document=random_document(),
        password=random_password(10),
        rol='admin'
...
def test_create_user_user_found(client: TestClient, superuser_token: dict[str, str], db: Session) -> None:
    admin_in = create_random_user('admin', db, 10)
    data = schemas.UserCreate(
        num_document=admin_in.num_document,
        password=random_password(10),
        rol='admin'
    )
...
def test_update_user(client: TestClient, superuser_token: dict[str, str], db: Session) -> None:
    ...
    user_search = schemas.UserSearch(
        num_document=admin.num_document, rol='admin'
    )
...
def test_update_user_non_superuser(client: TestClient, non_superuser_token: dict[str, str], db: Session) -> None:
    admin = create_random_user('admin', db, 10)
...
def test_update_user_user_not_found(client: TestClient, superuser_token: dict[str, str]) -> None:
    empty_info = schemas.UserUpdateAll()
...
def test_update_user_num_document_used(client: TestClient, superuser_token: dict[str, str], db: Session) -> None:
    admin_params = ('admin', db, 10)
    admin1 = create_random_user(*admin_params)
    admin2 = create_random_user(*admin_params)

    updated_info_admin1 = schemas.UserUpdateAll(
        num_document=admin2.num_document
    )
...
def test_delete_user(client: TestClient, superuser_token: dict[str, str], db: Session) -> None:
...
def test_delete_user_user_not_found(client: TestClient, superuser_token: dict[str, str]) -> None:
    response = client.delete(
        f'{endpoint}/{non_existent_document}/admin',
        headers=superuser_token
    )
...
def test_delete_user_non_superuser(client: TestClient, non_superuser_token: dict[str, str], db: Session) -> None:
    admin = create_random_user('admin', db, 10)
...
def test_delete_user_patient_in_bed(client: TestClient, superuser_token: dict[str, str], db: Session) -> None:
    hospitalization = create_random_hospitalization(db)
    response = client.delete(
        f'{endpoint}/{hospitalization.num_doc_patient}/patient',
        headers=superuser_token
    )
...
def test_get_user(client: TestClient, admin_token: dict[str, str], db: Session) -> None:
    ...
    response = client.get(
        f'{endpoint}/{user.num_document}?rol=true',
        headers=admin_token
    )
...
def test_get_user_user_not_found(client: TestClient, admin_token: dict[str, str]) -> None:
    response = client.get(
        f'{endpoint}/{non_existent_document}',
        headers=admin_token
    )
